var BoardBlast = {
	...
	toggleUser: function ( user_id ) {
		...
		if ( elem.hasClass( 'blast-family-selected' ) ) {
			elem.removeClass( 'blast-family-selected' )
				.addClass( 'blast-family-unselected' );
		} else if ( elem.hasClass( 'blast-family-unselected' ) ) {
			elem.removeClass( 'blast-family-unselected' )
				.addClass( 'blast-family-selected' );
		}
	},
	...
	toggleFamily: function ( method ) {
		BoardBlast.toggleType(
			method,
			'blast-family-selected',
			'blast-family-unselected'
		);
	},
	...
	sendMessages: function () {
		...
		list = $( '#blast-friends-list div.blast-family-selected' );
		for ( x = 0; x <= list.length - 1; x++ ) {
			el = list[ x ];
			selected++;
			user_id = el.id.replace( 'user-', '' );
...
$( () => {
	...
	// "Select/Unselect family" links
	$( 'div.blast-nav-links a.blast-select-family-link' ).on( 'click', () => {
		BoardBlast.toggleFamily( 1 );
	} );
	$( 'div.blast-nav-links a.blast-unselect-family-link' ).on( 'click', () => {
		BoardBlast.toggleFamily( 0 );
	} );
